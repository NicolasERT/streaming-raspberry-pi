...
# Imagen Docker FFmpeg con soporte WHIP (para VDO.Ninja)
if ! sudo docker image inspect linuxserver/ffmpeg &>/dev/null; then
    echo "📦 Descargando imagen linuxserver/ffmpeg (FFmpeg 8.1 con soporte WHIP)..."
    sudo docker pull linuxserver/ffmpeg
else
    echo "✅ Imagen linuxserver/ffmpeg ya disponible."
fi
